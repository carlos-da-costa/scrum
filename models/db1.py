...
def EDITAR(id,url):
    return A(TAG.button('Editar - %i' % id),_href=url+'/%i' % id)
...
db.define_table('sistema',
                Field('nome'))
db.sistema.id.represent = lambda id: A('Editar %i' % id,_href=URL(r=request,c='config',f='sistema',args=[id]))

db.define_table('cliente',
                Field('nome'))
db.cliente.id.represent =  lambda id: A('Editar %i' % id,_href=URL(r=request,c='config',f='cliente',args=[id]))

db.define_table('tipo_ticket',
                Field('nome'))
db.tipo_ticket.id.represent = lambda id: EDITAR(id,URL(r=request,c='config',f='tipo_ticket'))
...
db.define_table('ticket',
                ...
                Field('estado',db.estado_ticket),
                Field('patch','boolean'),
                Field('local_teste'),
                Field('inicio','date',requires=BRAZILIAN_DATE ),
                Field('fim','date',requires=BRAZILIAN_DATE   ),
                Field('sistema',db.sistema ),
...
db.ticket.sprint.requires = IS_EMPTY_OR(IS_IN_DB(db,db.sprint.id,'Sprint %(numero)s'))
db.ticket.sistema.requires = IS_IN_DB(db,db.sistema.id,'%(nome)s')
db.ticket.cliente.requires = IS_IN_DB(db,db.cliente.id,'%(nome)s')
